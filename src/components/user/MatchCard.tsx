...
export function MatchCard({ fixture, compact = false }: MatchCardProps) {
  ...
  return (
    <Link href={`/match/${fixture.id}`}>
      <div
        className={cn(
          'bg-white dark:bg-slate-800 rounded-xl border transition-all duration-200 hover:shadow-lg hover:border-orange-400',
          isLive ? 'border-orange-500 shadow-md bg-orange-50 dark:bg-orange-950/30' : 'border-gray-200 dark:border-slate-700',
          compact ? 'p-3' : 'p-4'
        )}
      >
        {/* Header */}
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            {fixture.round && (
              <span className="text-xs text-gray-500 dark:text-slate-400">{fixture.round}</span>
            )}
          </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 flex-1">
            ...
            <div>
              <p className={cn(
                'font-bold text-sm',
                fixture.winner_id === fixture.team_a_id ? 'text-green-700 dark:text-green-400' : 'text-gray-950 dark:text-slate-50'
              )}>
          ...
          <div className="px-4 text-center">
            {isLive ? (
              <div className="flex flex-col items-center">
                <span className="text-xs text-red-500 dark:text-red-400 dark:text-red-400 font-medium">
              ...
              <span className="text-xs text-gray-500 dark:text-slate-400">FT</span>
            ) : (
              <div className="flex flex-col items-center">
                <span className="text-xs text-gray-500 dark:text-slate-400">
          ...
          <div className="flex items-center gap-3 flex-1 justify-end">
            <div className="text-right">
              <p className={cn(
                'font-bold text-sm',
                fixture.winner_id === fixture.team_b_id ? 'text-green-700 dark:text-green-400' : 'text-gray-950 dark:text-slate-50'
              )}>
            ...
            <div className="w-10 h-10 rounded-full bg-gray-100 dark:bg-slate-700 flex items-center justify-center text-lg font-bold text-gray-950 dark:text-slate-100">
...
export function MatchCardMini({ fixture }: { fixture: FixtureWithDetails }) {
  ...
  return (
    <Link href={`/match/${fixture.id}`}>
      <div className={cn(
        'bg-white dark:bg-slate-800 rounded-lg border p-3 min-w-[200px] transition-all hover:shadow-sm',
        isLive ? 'border-red-200 dark:border-red-900/50' : 'border-gray-100 dark:border-slate-700'
      )}>
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs text-gray-500 dark:text-slate-400">{fixture.sport.name}</span>
          {isLive && (
